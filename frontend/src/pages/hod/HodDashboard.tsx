...
export function HodDashboard() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await api.get<MarksResponse[]>('/hod/marks/pending');
      setRows(res.data);
    } catch (err: any) {
  ...
  async function approve(row: MarksResponse) {
    try {
      await api.post(`/hod/marks/${row.id}/approve`, {});
      setRows((prev) => prev.filter((r) => r.id !== row.id));
      toast.push('success', `Approved ${row.studentName} — ${row.subjectCode}.`);
    } catch (err: any) {
      toast.push('error', err?.response?.data?.message || 'Could not approve.');
    }
  }
  ...
  async function confirmReject(reason: string) {
    ...
    try {
      await api.post(`/hod/marks/${rejectTarget.id}/reject`, { reason });
  ...
  return (
    <AppShell title="Pending Approvals">
      ...
      {!loading && rows.length === 0 && <p style={{ color: 'var(--color-ink-soft)' }}>Nothing awaiting your review.</p>}
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} style={{ borderBottom: '1px solid var(--color-line)' }}>
                ...
                <td style={td} className="mono-num">{row.iaMarks}</td>
                <td style={td} className="mono-num">{row.externalMarks}</td>
                <td style={td} className="mono-num">{row.totalMarks}</td>
                <td style={td}><StatusStamp status={row.status} /></td>
                <td style={td}>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button style={primaryBtn} onClick={() => approve(row)}>Approve</button>
